...
public class DatabaseRepositorySQLiteImpl implements DatabaseRepository {
    ...
    @Override
    public ArrayList<WeatherData> getAllSavedData() throws SQLException {
        try (Connection connection = getConnection();
             Statement statement = connection.createStatement()) {
             ResultSet resultSet = statement.executeQuery("SELECT * FROM weather");
             ArrayList<WeatherData> listWeather = new ArrayList<WeatherData>();
             while (resultSet.next()) {
                 listWeather.add(new WeatherData(resultSet.getString(2),resultSet.getString(3),resultSet.getString(4),resultSet.getDouble(5)));
             }
             return listWeather;
           } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        throw new SQLException("Not implemented exception");
    }
}
